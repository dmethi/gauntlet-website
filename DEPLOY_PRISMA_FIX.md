...
The live odds cron job has been **running but failing silently** due to a Prisma binary mismatch. 
...
But **investigate first** - this fix is correct based on Prisma + Vercel best practices.
...
### "Why did this happen?"
Prisma defaults to generating only the binary for your current OS (macOS). Vercel runs on AWS Lambda (RHEL), which needs a different binary. We never specified which binaries to include, so the Linux one was missing.
...
The recap report cron (`/api/cron/recap-report`) uses the same Prisma client, so it's also fixed by this change.
...
*Last Updated: 2025-10-09 00:30 PST*  
