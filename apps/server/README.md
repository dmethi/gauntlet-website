...
This package contains **background job scripts** and **data ingestion pipelines** that are executed by GitHub Actions workflows. The HTTP server routes (`src/routes/*`) are **UNUSED** - the web app uses Next.js API routes instead.
...
- `comprehensive-live-snapshot.ts` - Captures live matchup data with win probabilities
- Runs Monte Carlo simulations for current matchups
- Stores historical odds snapshots
- **Triggered by:** `.github/workflows/live-odds-updates.yml` (every 10 min during NFL games)
...
- **Triggered by:** `.github/workflows/live-odds-updates.yml` (before each snapshot)
...
### Key Insight
- **Web app:** Database-free, API-first (Sleeper API → Next.js API routes → Frontend)
...
### 1. Live Odds Updates
**File:** `.github/workflows/live-odds-updates.yml`
**Schedule:** Every 10 minutes during NFL game windows
**Commands:**
```bash
...
### 2. Live Simulations
**File:** `.github/workflows/live-sims.yml`
**Schedule:** Every 10 minutes during NFL game windows
**Commands:**
```bash
...
**Schedule:** Daily at 8am ET during NFL season
**Commands:**
```bash
...
**A:** Historical data storage for analytics and trends. Background jobs store simulation results, live odds history, and player stats over time.
...
**A:** Legacy artifact. It's not needed anymore and can be removed from `apps/web/package.json`.

## Related Documentation

- [PRISMA_CLEANUP_ANALYSIS.md](../../PRISMA_CLEANUP_ANALYSIS.md) - Detailed analysis of database usage
