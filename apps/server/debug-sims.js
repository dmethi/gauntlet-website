...
async function debugSimResults() {
  ...
  matchups.forEach((team, i) => {
    ...
    console.log(
      `  Team ${i + 1} (roster ${team.rosterId}): ${team.points} current → ${simMean?.toFixed(1)} expected`
    );

    const diff = team.points - simMean;
    if (Math.abs(diff) > 10) {
      console.log(
        `    ❌ PROBLEM: ${diff > 0 ? 'Current higher' : 'Expected higher'} by ${Math.abs(diff).toFixed(1)} pts`
      );
    }
  });
