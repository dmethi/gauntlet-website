...
async function ingestLeague(ctx: IngestContext) {
...
async function ingestUsers(ctx: IngestContext) {
  const { leagueId } = ctx.options;
...
async function ingestRosters(ctx: IngestContext) {
  const { leagueId } = ctx.options;
...
async function ingestMatchups(ctx: IngestContext, week: number) {
  const { leagueId } = ctx.options;
...
async function ingestWeeklyPlayerStats(ctx: IngestContext, season: string, week: number) {
...
async function ingestWeeklyProjections(ctx: IngestContext, season: string, week: number) {
  const seasonNum = Number(season);
...
async function ingestPlayers(ctx: IngestContext) {
...
async function ingestDraft(ctx: IngestContext, draftId: string) {
  ...
  const slotToRosterId = draft.slot_to_roster_id
    ? Object.values(draft.slot_to_roster_id).map((id: any) => Number(id))
    : [];
...
async function ingestTransactions(ctx: IngestContext, week: number) {
  const { leagueId } = ctx.options;
...
async function ingestAll(options: Partial<IngestContext['options']> = {}) {
...
if (isDirectRun) {
  ...
  const season = process.env.INGEST_SEASON || DEFAULT_OPTIONS.season || '2025';
  const weeksEnv = process.env.INGEST_WEEKS; // e.g., "1-18" or "1,2,3"

  let weeks = DEFAULT_OPTIONS.weeks;
  if (weeksEnv) {
