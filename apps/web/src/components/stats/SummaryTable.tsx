...
export const SummaryTable = ({ data, showLeagueRank = true }: SummaryTableProps) => {
  ...
  return (
    <Table surface="responsive" scrollLabel="Team scoring summary">
      <TableHeader>
        <TableRow>
      ...
      <TableBody>
        {sortedData.map((row, index) => (
          <TableRow key={row.teamName}>
            ...
            <TableCell className="text-right font-mono">{formatNumber(row.windowTotal)}</TableCell>
            ...
            {showLeagueRank && (
              <TableCell className="text-center">
                <Badge
                  variant={
                    row.rankLeague <= 2 ? 'default' : row.rankLeague <= 6 ? 'secondary' : 'outline'
                  }
                  className="min-w-[2.5rem] justify-center"
                >
                  {row.rankLeague}
