...
import { ALL_HALL_OF_FAME_CATEGORIES, getCategoriesGrouped } from './categories';
 ...
 */
export const calculateHallOfFameRecords = (
 ...
 * Note: Only searches base categories. For expanded categories, use getAllCategories from categories-expanded.ts
 */
export const getCategoryInfo = (categoryId: string): HallOfFameCategory | undefined => {
  return HALL_OF_FAME_CATEGORIES.find(c => c.id === categoryId);
};

/**
 * Group categories by their group type
 */
export const getCategoriesByGroup = (): Map<string, HallOfFameCategory[]> => {
 ...
 */
export const formatRecord = (record: HallOfFameRecord): string => {
