...
const findTiebreakerCompetitors = (
  ...
  sameRecordTeams.forEach(competitor => {
    ...
    if (competitorMatchup) {
      ...
      competitorMustWin =
        (competitorIsTeam1 && winner === 'team1') || (!competitorIsTeam1 && winner === 'team2');
    }
...
const buildAllPathsForTeam = (
  ...
  allOutcomes.forEach((outcome, outcomeId) => {
    ...
    const conditions = buildPathConditions(outcome, matchups, rosterId, standings, baseSeed);
  ...
  if (winPathCount === 32 && losePathCount === 0) {
    ...
    if (winPathCount >= losePathCount) {
      ...
      if (loseConditions.length > 0)
        additionalConditions.push(`LOSE: ${loseConditions.join(', ')}`);
    } else {
      ...
      if (seedScenarios.length === 0) {
        ...
        if (winOutcomes.length > 0 && loseOutcomes.length > 0) {
          ...
          const conditionCounts = new Map<
            string,
            { count: number; condition: ScenarioCondition }
          >();
