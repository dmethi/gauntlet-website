...
export const NFL_TEAM_CODES = NFL_TEAMS.map(team => team.code) as [
  (typeof NFL_TEAMS)[number]['code'],
  ...(typeof NFL_TEAMS)[number]['code'][],
];

export const getNflTeamName = (code: string | null): string | null =>
  NFL_TEAMS.find(team => team.code === code)?.name ?? null;
