...
const DEFAULT_LEAGUE_ID = getCurrentLeagues()[0]?.id ?? '';
  ...
  const formattedTeamStats =
    teamStats?.map(team => {
      ...
      return {
        ...
        wins: Number((roster?.settings as any)?.wins ?? team.wins),
        losses: Number((roster?.settings as any)?.losses ?? team.losses),
