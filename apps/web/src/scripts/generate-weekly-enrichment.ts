...
function processPositionalTrends(dataset: any): Narrative[] {
  ...
  for (const position of positions) {
    ...
    for (const [teamKey] of posTeamsMap.entries()) {
      const ranks: number[] = [];
    ...
    for (const [teamKey, scores] of teamWeekScores.entries()) {
      ...
      const previousAvgPoints = scores.previous.reduce((sum, p) => sum + p, 0) / scores.previous.length;
      ...
      for (let week = 1; week < currentWeekNum; week++) {
        ...
        const weekScore = posTeamsMap.get(teamKey)?.scores.find((s: any) => s.week === week);
...
function processLuckAnalysis(dataset: any): Narrative[] {
  ...
  for (const [teamKey, teamData] of teamsMap.entries()) {
    ...
    for (const [_, otherTeam] of teamsMap.entries()) {
    ...
    const scheduleStrength = opponentScores.reduce((sum: number, s: any) => sum + s.value, 0) / opponentScores.length;
    ...
    for (const [key, team] of teamsMap.entries()) {
      ...
      if (oppScores.length > 0) {
        const strength = oppScores.reduce((sum: number, s: any) => sum + s.value, 0) / oppScores.length;
...
function processScatterOutliers(dataset: any): Narrative[] {
  ...
  for (const chartType of positions) {
    ...
    for (const [teamKey, teamData] of teamsMap.entries()) {
      ...
      const xValue = teamScores.reduce((sum: number, s: any) => sum + s.value, 0) / teamScores.length;
...
async function generateEnrichment() {
  try {
    ...
    const filterAndLimit = (narratives: Narrative[], maxCount: number) => {
      return narratives
        .filter(n => {
          ...
          if (NARRATIVE_CONFIG.minSeverity === 'moderate') return n.severity === 'critical' || n.severity === 'moderate';
          return true;
        })
        .sort((a, b) => {
    ...
    const enrichment = {
      ...
      summary: {
        totalNarratives: positionalTrends.length + luckAnalysis.length + scatterOutliers.length + transactions.length + startSit.length,
        bySection: {
      ...
      narratives: {
        positionalTrends: filterAndLimit(positionalTrends, NARRATIVE_CONFIG.maxNarrativesPerSection.positionalTrends),
        luckAnalysis: filterAndLimit(luckAnalysis, NARRATIVE_CONFIG.maxNarrativesPerSection.luckAnalysis),
        scatterOutliers: filterAndLimit(scatterOutliers, NARRATIVE_CONFIG.maxNarrativesPerSection.scatterOutliers),
        transactions: filterAndLimit(transactions, NARRATIVE_CONFIG.maxNarrativesPerSection.transactions),
