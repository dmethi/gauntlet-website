...
- **Search `docs/solutions/` and read `docs/solutions/patterns/critical-patterns.md` when relevant; apply learnings before coding.**
...
5. **Submit** — PR must solve one logical problem, include a clear description, and pass CI.

## Dependency Policy

Before adding a dependency, answer: What problem does this solve? Is there an existing solution in the repo? Can we implement a minimal version ourselves? What is the maintenance risk? If not justified → do not add it.
