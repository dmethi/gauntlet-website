...
A groomed issue is **execution-ready**. An agent (or developer) can pick it up
and produce a mergeable PR without making design decisions or asking clarifying
questions.
...
- **If no, why**: [brief reason: research needed, ambiguous requirements,
  complex refactor]
...
- **Patterns to follow**: [reference existing code, e.g., "follow pattern in
  `src/components/ExistingThing.tsx`"]
...
- [ ] **Autocomplete readiness captured** — yes/no, steps, verification (or
      reason if no)
...
1. **Keep issues small** — If you need extensive code snippets, the issue is
   probably too big
2. **Reference, don't repeat** — Point to existing patterns instead of
   re-explaining them
3. **Be specific on boundaries** — "Out of Scope" prevents scope creep during
   execution
4. **Calibrate risk honestly** — Underestimating blast radius causes production
   incidents
5. **Think like the executor** — What would you need to know to do this without
   asking questions?
