...
Successfully implemented 6 new matchup-related Hall of Fame categories as
requested. These categories analyze entire matchups rather than individual team
performances, providing insights into the most exciting, lopsided, and memorable
...
1. **Most Exciting Game** (`most_exciting_game`)
   ...
   - **Description**: Using 10-minute win probability swings, total points, and
     margin composite score
   - **Current Implementation**: Composite score of total points (40%) + inverse
     margin (60%)
   - **TODO**: Enhance with LiveWinProbSample data for true win probability
     swings
...
6. **Most One-Sided Game** (`most_one_sided_game`)
   - **Type**: High (Top 5 only)
   - **Description**: Combination of large margin and low loser score
     (embarrassing defeats)
   - **Implementation**: `margin × (1.5 - (loser_score / 150))` - amplifies
     margins when loser scored very few points

## Technical Implementation

### Database Schema

- **Categories**: Added to `HallOfFameCategory` table with proper groupName and
  sortOrder
- **Records**: Will be stored in `HallOfFameRecord` table with matchup context
  data
- **Group**: "Weekly – Matchup Subject" (new group E)

### Data Sources

- **Primary**: `MatchupSummary` table for basic matchup data (points, margin,
  winner)
- **Future**: `LiveWinProbSample` table for 10-minute win probability snapshots
...
1. **Winner-Centric Tracking**: Matchup records tracked via winning team for
   consistency
2. **Rich Context Data**: Full matchup details preserved for UI display
   flexibility
3. **Placeholder TODOs**: Excitement algorithm ready for LiveWinProbSample
   integration
4. **Normalized Scoring**: Algorithms use normalized values (0-1 scales) for
   fair comparison
5. **Formula Documentation**: Complex calculations include formula explanations
   in context data
