...
The Sleeper API includes undocumented endpoints for accessing player projections
that complement the stats endpoints. These provide projected performance data
for upcoming games and seasons.
...
Similar to stats, returns a JSON object with player IDs as keys and projection
objects as values.
