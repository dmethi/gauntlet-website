...
async function collectSampleHistoricalData(availabilityResults) {
  ...
  const viableSeasons = availabilityResults.filter(season => 
    season.statsAvailable.length >= 2 && season.projectionsAvailable.length >= 2
  );
...
function analyzeWeekData(statsData, projectionsData) {
  ...
  for (const [position, data] of Object.entries(analysis.positions)) {
    ...
    const variance = data.scores.reduce((sum, score) => sum + Math.pow(score - mean, 2), 0) / data.count;
...
function calculatePositionVariance(weekData) {
  ...
  for (const [position, data] of Object.entries(positionStats)) {
    ...
    const variance_calc = allScores.reduce((sum, score) => sum + Math.pow(score - mean, 2), 0) / allScores.length;
...
function calculateWeeklyConsistency(weeklyMeans) {
  ...
  const variance = weeklyMeans.reduce((sum, val) => sum + Math.pow(val - mean, 2), 0) / weeklyMeans.length;
...
function deriveSimulationConstants(sampleData) {
  ...
  for (const [position, data] of Object.entries(aggregatedVariance)) {
    const avgCoefficient = data.coefficients.reduce((sum, c) => sum + c, 0) / data.coefficients.length;
    ...
    console.log(`${position}: CV=${avgCoefficient.toFixed(3)}, Avg=${avgMean.toFixed(1)} pts, n=${totalSamples}`);
  }
