  ...
  for (const result of statsResults) {
    ...
    if (isProjection) {
      analysis.projections[result.description] = {
        ...
        sampleEntries: Object.entries(data).slice(0, 2).map(([id, stats]) => ({ id, stats }))
      };
...
async function saveResults(allResults, analysis) {
  ...
  try {
    ...
    const summary = {
      ...
      endpointTypes: {
        publicEndpoints: allResults.filter(r => r.description.includes('NFL State') || r.description.includes('Trending')).length,
