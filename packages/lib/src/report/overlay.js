export function computeVolatilityLabel(input) {
    ...
    if (leadChanges >= 10 || maxSwingPct >= 8 || avgDeltaPct >= 5)
        return 'Chaos';
    if (leadChanges >= 4 || maxSwingPct >= 4 || avgDeltaPct >= 2)
        return 'Balanced';
    return 'Chalk';
}
export function buildMatchupOverlay(inputs) {
    const { teamAName, teamBName, leadChanges, avgDeltaPct, maxSwingPct = 0, startersSumA = 0, startersSumB = 0, benchSumA = 0, benchSumB = 0, qbA = 0, qbB = 0, defA = 0, defB = 0, } = inputs;
    const outcome = {
        volatility: computeVolatilityLabel({ leadChanges, avgDeltaPct, maxSwingPct }),
    };
    // Deterministic variety via seed
    const seed = Math.abs((teamAName + '|' + teamBName + '|' + String(inputs.matchupId))
        .split('')
    ...
    if (!outcome.fineText) {
        if (aBenchHeavy)
            outcome.fineText = pick(benchLinesA);
        else if (bBenchHeavy)
            outcome.fineText = pick(benchLinesB);
        else if (startersDelta >= lineupGapThreshold)
            outcome.fineText = pick(malpracticeLines);
        else if (rand(0, 1) > 0.7)
            outcome.fineText = pick(hubrisLines);
    }
    ...
    if (qbGap >= qbGapThresh) {
        ...
        if (qbA > qbB)
            outcome.curseTextB = line(qbA, qbB);
        else
            outcome.curseTextA = line(qbB, qbA);
    }
    if (defGap >= defGapThresh) {
        ...
        if (defA > defB && !outcome.curseTextB)
            outcome.curseTextB = line(defA, defB);
        else if (defB > defA && !outcome.curseTextA)
            outcome.curseTextA = line(defB, defA);
    }
    return outcome;
}
