...
## Overview
Extract all TypeScript interfaces and types from `manager-analysis.tsx` to a dedicated `types.ts` file.

## Context Needed
- File: `apps/web/src/components/manager-analysis.tsx` (lines 46-48 ONLY - just the props interface)
- Reference: `apps/web/src/lib/manager-analytics.ts` (types are imported from here)
...
// NEW:
import type { ManagerAnalytics, ManagerProfile, PlayerOverlap } from '@/lib/manager-analytics';
import type { ManagerAnalysisProps, SortConfig, SortKey } from '@/features/manager-analysis/types';
